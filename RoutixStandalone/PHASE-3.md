...
- Large typographic speed display and floating map controls above the bottom dock. The route previews, map overlays and comparison UI use theme tokens. Default route color follows the chosen accent; explicit custom trace/marker colors remain supported.
...
GitHub Actions runs the existing navigation/storage tests and Robolectric screen rendering checks at 320, 390 and 430 dp. Added contrast checks for all 56 flavor/accent combinations and map appearance preference checks. Populated route folders, detail, guidance, settings, recording, hours and GPX screens are rendered in all four flavors, plus light/dark onboarding. Rendered PNGs are retained with the build artifacts. Real GPS driving, direct sunlight and physical-device battery use require field validation; layout tests do not establish those properties.
