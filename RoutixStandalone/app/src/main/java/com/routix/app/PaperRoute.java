...
final class PaperRoute {
        ...
        for(Road road:roads)for(int j=1;j<road.points.size();j++){
            ...
            for(int x=(int)Math.floor((Math.min(a.x,b.x)-35)/100);x<=(int)Math.floor((Math.max(a.x,b.x)+35)/100);x++)
                for(int y=(int)Math.floor((Math.min(a.y,b.y)-35)/100);y<=(int)Math.floor((Math.max(a.y,b.y)+35)/100);y++)
                    grid.computeIfAbsent(x+":"+y,k->new ArrayList<>()).add(segment);
        }
        List<Step> out=new ArrayList<>();String previous=null;
        for(int i=0;i<trace.size();i++){
            Point p=trace.get(i),before=trace.get(Math.max(0,i-1)),after=trace.get(Math.min(trace.size()-1,i+1));
            double ux=after.x-before.x,uy=after.y-before.y,len=Math.hypot(ux,uy);
            Road best=null;double score=Double.POSITIVE_INFINITY,second=Double.POSITIVE_INFINITY;
            for(Road road:grid.getOrDefault((int)Math.floor(p.x/100)+":"+(int)Math.floor(p.y/100),Collections.emptyList())){
                ...
                for(int j=1;j<road.points.size();j++){
                    Point a=road.points.get(j-1),b=road.points.get(j);
                    if(p.x<Math.min(a.x,b.x)-35||p.x>Math.max(a.x,b.x)+35||p.y<Math.min(a.y,b.y)-35||p.y>Math.max(a.y,b.y)+35)continue;
                    double dist=segmentDistance(p,a,b);if(dist>35)continue;
                    double vx=b.x-a.x,vy=b.y-a.y,vl=Math.hypot(vx,vy);
                    double alignment=len<2||vl==0?1:Math.abs((ux*vx+uy*vy)/(len*vl));
                    roadScore=Math.min(roadScore,dist+12*(1-alignment));
                }
                if(roadScore<score){if(best!=null&&!road.key.equals(best.key))second=score;best=road;score=roadScore;}
                else if(best!=null&&!road.key.equals(best.key))second=Math.min(second,roadScore);
            }
    ...
    static List<int[]> sheets(List<Point> trace,List<Step> steps){
        ...
        while(start<trace.size()-1){
            ...
            for(;end<trace.size();end++){
                Point q=trace.get(end);double nx=Math.min(minX,q.x),xx=Math.max(maxX,q.x),ny=Math.min(minY,q.y),xy=Math.max(maxY,q.y);
                boolean change=false;for(Step s:steps)if(s.index==end){change=true;break;}
                if(end>start+1&&(Math.max(xx-nx,xy-ny)>1200||(change&&count>=9)))break;
