...
public final class TrackingService extends Service implements LocationListener {
    ...
    private void write(Runnable task){io.execute(()->{try{task.run();}catch(RuntimeException e){DiagnosticLog.error("session persistence",e);main.post(()->android.widget.Toast.makeText(this,"Sauvegarde impossible : vérifie le stockage. Exporte le diagnostic.",android.widget.Toast.LENGTH_LONG).show());}});}
    ...
    private void installGuidance(RouteStore.Summary s,float progress){route=s;List<GuidanceEngine.Point> p=new ArrayList<>();for(RouteStore.Point x:s.points)p.add(new GuidanceEngine.Point(x.lat,x.lon));List<GuidanceEngine.Event> e=new ArrayList<>();for(RouteStore.Event x:s.events){int index=-1;if(x.time>0){long best=Long.MAX_VALUE;for(int i=0;i<s.points.size();i++){long t=s.points.get(i).time;if(t>0&&Math.abs(t-x.time)<best){best=Math.abs(t-x.time);index=i;}}}e.add(new GuidanceEngine.Event(x.type,x.label,x.lat,x.lon,index));}guidance=new GuidanceEngine(p,e);guidance.restoreProgress(progress);guidanceState=null;spoken=null;}
    void addGuidanceEvent(String type,String label){if(route==null||location==null)return;RouteStore.Summary current=route;float progress=guidance.progressM();RouteStore.Event event=new RouteStore.Event(type,label,location.getLatitude(),location.getLongitude(),System.currentTimeMillis(),location.getAccuracy());current.events.add(event);installGuidance(current,progress);List<RouteStore.Event> copy=new ArrayList<>(current.events);write(()->{if(!store.writeGpx(current.file,current.points,copy,current.firstTime,store.displayName(current.file)))throw new IllegalStateException("Marker save failed");});publish();}
    void stopGuidance(){guidance=null;guidanceState=null;route=null;write(()->journal.state("guidance",""));idle();publish();}
    boolean reposition(){if(location==null||guidance==null||location.getAccuracy()>35)return false;boolean ok=guidance.reposition(location.getLatitude(),location.getLongitude());if(ok){float progress=guidance.progressM();write(()->journal.state("progress",Float.toString(progress)));onLocationChanged(location);}return ok;}
    private void idle(){if(!recording&&guidance==null){if(foreground){stopForeground(STOP_FOREGROUND_REMOVE);foreground=false;stopSelf();}if(!visible)unsubscribe();}}
    void saveDraft(){if(!recording)return;List<RouteStore.Point> p=new ArrayList<>(points);List<RouteStore.Event> e=new ArrayList<>(events);long start=started;write(()->store.saveDraft(p,e,start));}
    ...
    @Override public void onLocationChanged(Location fix){
        ...
        try{if(guidance!=null){guidance.setOffRouteThreshold(prefs.getInt("offroute_m",45));guidanceState=guidance.update(fix.getLatitude(),fix.getLongitude(),fix.getElapsedRealtimeNanos()/1000000,fix.getAccuracy());float progress=guidance.progressM();write(()->journal.state("progress",Float.toString(progress)));GuidanceEngine.Event next=guidanceState.nextEvent;
            if(next!=null&&next!=spoken&&!guidanceState.poorAccuracy&&!guidanceState.offRoute&&guidanceState.distanceToNextEventM<55&&prefs.getBoolean("voice_markers",true)&&speechReady){speech.speak(next.label+" dans "+Math.round(guidanceState.distanceToNextEventM)+" mètres",TextToSpeech.QUEUE_FLUSH,null,"marker");spoken=next;}
    ...
    @Override public void onProviderDisabled(String provider){DiagnosticLog.info("GPS disabled");}
    @Override public void onProviderEnabled(String provider){DiagnosticLog.info("GPS enabled");}
    @Override public void onDestroy(){closed=true;listener=null;main.removeCallbacksAndMessages(null);unsubscribe();if(speech!=null)speech.shutdown();io.execute(journal::close);io.shutdown();super.onDestroy();}
}
