...
public final class TrackingService extends Service implements LocationListener {
    ...
    private Notification trackingNotification(){
        ...
        NotificationCompat.Builder b=new NotificationCompat.Builder(this,"tracking").setSmallIcon(R.drawable.ic_tracking).setColor(CatppuccinTheme.from(this).accent).setContentTitle(title).setContentText(text).setContentIntent(pending).setOngoing(true).setOnlyAlertOnce(true).setSilent(true).setVisibility(NotificationCompat.VISIBILITY_PUBLIC).setCategory(NotificationCompat.CATEGORY_NAVIGATION);\n        if(guidance!=null&&guidanceState!=null){String summary=formatIslandDistance(guidanceState.remainingM)+" · "+progress+" %";b.setSubText(summary);b.getExtras().putCharSequence("android.shortCriticalText",formatIslandDistance(guidanceState.remainingM));}
