...
public final class TrackingService extends Service implements LocationListener {
    ...
    void beginGuidance(RouteStore.Summary s){if(!ready||s==null||s.points.size()<2||!activate())return;
        guidanceStartedAt=System.currentTimeMillis();guidanceTravelDistance=0;guidanceReverseAdded=0;guidanceTwoSidesAdded=0;lastStatsWriteMs=0;
    ...
    void stopGuidance(){persistGuidanceStats(true);guidance=null;guidanceState=null;route=null;write(()->{journal.state("guidance","");journal.state("progress","0");journal.state("guidance_started","0");journal.state("guidance_distance","0");journal.state("guidance_reverse","0");journal.state("guidance_two_sides","0");});releaseSpeech();guidanceStartedAt=0;guidanceTravelDistance=0;guidanceReverseAdded=0;guidanceTwoSidesAdded=0;idle();publish();}
    ...
    @Override public void onLocationChanged(Location fix){
        ...
        try{if(guidance!=null){
            if(previous!=null&&previous.getAccuracy()<=45&&fix.getAccuracy()<=45){float[] travelled=new float[1];Location.distanceBetween(previous.getLatitude(),previous.getLongitude(),fix.getLatitude(),fix.getLongitude(),travelled);if(travelled[0]>=2.5f&&travelled[0]<250)guidanceTravelDistance+=travelled[0];}
            guidance.setOffRouteThreshold(prefs.getInt("offroute_m",45));guidanceState=guidance.update(fix.getLatitude(),fix.getLongitude(),fix.getElapsedRealtimeNanos()/1000000,fix.getAccuracy());float progress=guidance.progressM();persistProgress(progress,false);persistGuidanceStats(false);GuidanceEngine.Event next=guidanceState.nextEvent;
